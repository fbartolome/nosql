...
public class JanusQueryRunner {
    ...
    private static void query2(final JanusGraph graph) {
        ...
        final GraphTraversal<Vertex, Path> result = g.V()
                ...
                .until(out(HAS_VENUE_EDGE).out(HAS_SUBCATEGORY_EDGE).out(HAS_CATEGORY_EDGE)
                        .has(CATEGORY_PROPERTY, "Airport"))
                .as("finish")
    ...
    private static void query3(final JanusGraph graph) {
        final GraphTraversalSource g = graph.traversal();
        final GraphTraversal<Vertex, Path> result = g.V().group(USER_ID_PROPERTY)
                .hasLabel(STOP_VERTEX).as("end")
                .out(HAS_VENUE_EDGE)
                .in(HAS_VENUE_EDGE).as("start")
                .filter(select("start", "end").where("start", q3Filter("end")))
                .repeat(out(HAS_STEP_EDGE))
                .until(where(eq("end")))
                .path();
    ...
    private static P q3Filter(final Object value) {
        return P.test((o, o2) -> {
