...
public class JanusPopulator {
  ...
  public static final String STEP_TYPE_EDGE = "trajStep";
  public static final String IS_VENUE_EDGE = "isVenue";
  public static final String HAS_CATEGORY_EDGE = "hasCategory";
  public static final String SUBCATEGORY_OF_EDGE = "subCategoryOf";
  ...
  private static void createSchema(final JanusGraph graph) {
    ...
    management.makeEdgeLabel(STEP_TYPE_EDGE).multiplicity(MANY2ONE).make();
    management.makeEdgeLabel(IS_VENUE_EDGE).multiplicity(MANY2ONE).make();
    management.makeEdgeLabel(HAS_CATEGORY_EDGE).multiplicity(MANY2ONE).make();
    management.makeEdgeLabel(SUBCATEGORY_OF_EDGE).multiplicity(MANY2ONE).make();
  ...
  private static Map<String, Vertex> populateVenues(final JanusGraphTransaction transaction,
    ...
    for (final Venue venue : venues.values()) {
      ...
      if (subcategoriesVertices.containsKey(venue.getCategory())) {
        subcategoryVertex = subcategoriesVertices.get(venue.getCategory());
      } else {
        subcategoryVertex = transaction
            .addVertex(T.label, SUBCATEGORY_VERTEX, SUBCATEGORY_PROPERTY, venue.getCategory());
        subcategoriesVertices.put(venue.getCategory(), subcategoryVertex);
      }

      if (categoriesVertices.containsKey(venue.getCattype())) {
        categoryVertex = categoriesVertices.get(venue.getCattype());
      } else {
        categoryVertex = transaction.addVertex(T.label, CATEGORY_VERTEX, CATEGORY_PROPERTY, venue.getCattype());
        categoriesVertices.put(venue.getCattype(), categoryVertex);
      }

      // Add relationships
      venueVertex.addEdge(HAS_CATEGORY_EDGE, subcategoryVertex);
      venueVertices.put(venue.getId(), venueVertex);
      if (!subcategoriesVertices.keySet().contains(venue.getCategory())) {
        subcategoryVertex.addEdge(SUBCATEGORY_OF_EDGE, categoryVertex);
      }
    }
  ...
  private static void populateTrajectories(final JanusGraphTransaction transaction, final List<Trajectory> trajectories,
    ...
    for (final Trajectory trajectory : trajectories) {
      ...
      for (final Visit visit : trajectory.getVisits()) {
        ...
        final Vertex stopVertex = transaction.addVertex(T.label, STOP_VERTEX);
        stopVertex.property(USER_ID_PROPERTY, userId);
        stopVertex.property(TIMESTAMP_PROPERTY, timestamp);
        stopVertex.property(VISIT_INDEX_PROPERTY, visitNumber++);
        stopVertex.addEdge(IS_VENUE_EDGE, venueVertex);
      }
    }
  }
